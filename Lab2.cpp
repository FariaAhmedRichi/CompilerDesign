...
bool isKeyword(const char* str) {
    if (!std::strcmp(str, "if") || !std::strcmp(str, "else") ||
        ...
        !std::strcmp(str, "for")|| !std::strcmp(str, "endl;"))
...
void parse(const char* str) {
    ...
    while (right <= len && left <= right) {
        ...
        if (isSymbol(str[right]) == true && left == right) {
            ...
            else if (isSymbol(str[right]) == true && left != right || (right == len && left != right)) {
            char* subStr = subString(str, left, right - 1);

            if (isPunctuation(str[right]) == true){
                std::cout << "'" << str[right] << "' Is a Punctuation" << std::endl;
            right++;
            left = right;}
            ...
            if (isKeyword(subStr) == true)
                ...
                std::cout << "'" << subStr << "' Is a Number" << std::endl;
            else if (isConstant(subStr) == true)
                std::cout << "'" << subStr << "'  Is a Constant" << std::endl;
            else if (validIdentifier(subStr) == true && isSymbol(str[right - 1]) == false)
                std::cout << "'" << subStr << "' Is a Valid Identifier" << std::endl;
            else if (validIdentifier(subStr) == false && isSymbol(str[right - 1]) == false)
                std::cout << "'" << subStr << "' Is a Constant" << std::endl;
                else
                    std::cout<<"'"<<subStr<<"' Is a Consatnt"<<std::endl;
            free(subStr);
            left = right;
        }
    }
...
int main() {
    ...
    std::ifstream inputFile("TestExpession.txt");
